...
{
    public class MasterActivityTypeRepository : IMasterActivityTypeRepository
    {
        ...
        private const string EntityName = AuditConstants.Entities.MasterActivityType;

        public MasterActivityTypeRepository(IOptionsMonitor<AppSettingsModel> options, IMasterAuditLogRepository masterAuditLogRepository)
        {
        ...
        public List<MasterActivityTypeResponseModel> GetAll(out string errorMessage)
        {
            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetAllMasterActivityTypes,
                new Dictionary<string, object>(),
                out errorMessage);

            return dt.ToList<MasterActivityTypeResponseModel>();
        }
        ...
        public MasterActivityTypeResponseModel GetById(int id, out string errorMessage)
        {
            ...
            {
                { "MasterActivityTypeId", id }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetMasterActivityTypeById,
                param,
                out errorMessage);

            if (dt.Rows.Count > 0)
            {
                return dt.ToList<MasterActivityTypeResponseModel>().FirstOrDefault();
            }
        ...
        public int AddOrUpdate(MasterActivityTypeRequestModel request, out string errorMessage)
        {
            ...
            var action = request.MasterActivityTypeId.HasValue && request.MasterActivityTypeId > 0
                ? AuditConstants.Actions.Update
                : AuditConstants.Actions.Insert;
       
            var param = new Dictionary<string, object>
            {
                { "MasterActivityTypeId", request.MasterActivityTypeId ?? (object)DBNull.Value },
                { "Name", request.Name },
                { "CreatedBy", request.CreatedBy }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_UpsertMasterActivityType,
                param,
                out errorMessage);
            ...
            if (!string.IsNullOrWhiteSpace(errorMessage))
            {
                var errorAuditRequest = new MasterAuditLogRequestModel
                {
                    ...
                    EntityId = request.MasterActivityTypeId?.ToString(),
        ...
        public int Delete(DeleteRequest deleteRequest, out string errorMessage)
        {
            ...
            var param = new Dictionary<string, object>
            {
                { "MasterActivityTypeId", deleteRequest.Id },
                { "DeletedReason", deleteRequest.Reason },
                { "DeletedBy", deleteRequest.DeletedBy }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_DeleteMasterActivityType,
                param,
                out errorMessage);
