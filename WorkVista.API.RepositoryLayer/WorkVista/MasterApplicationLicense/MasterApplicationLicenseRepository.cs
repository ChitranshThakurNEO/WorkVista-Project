...
{
    public class MasterApplicationLicenseRepository : IMasterApplicationLicenseRepository
    {
        ...
        private const string EntityName = AuditConstants.Entities.MasterApplicationLicense;

        public MasterApplicationLicenseRepository(IOptionsMonitor<AppSettingsModel> options, IMasterAuditLogRepository masterAuditLogRepository)
        {
        ...
        public List<MasterApplicationLicenseResponseModel> GetAll(out string errorMessage)
        {
            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetAllMasterApplicationLicenses,
                new Dictionary<string, object>(),
                out errorMessage);

            return dt.ToList<MasterApplicationLicenseResponseModel>();
        }
        ...
        public MasterApplicationLicenseResponseModel GetById(int id, out string errorMessage)
        {
            ...
            {
                { "MasterApplicationLicenseId", id }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetMasterApplicationLicenseById,
                param,
                out errorMessage);

            if (dt.Rows.Count > 0)
                return dt.ToList<MasterApplicationLicenseResponseModel>().FirstOrDefault();
        ...
        public int AddOrUpdate(MasterApplicationLicenseRequestModel request, out string errorMessage)
        {
            ...
            var action = request.MasterApplicationLicenseId.HasValue && request.MasterApplicationLicenseId > 0
                ? AuditConstants.Actions.Update
                : AuditConstants.Actions.Insert;

            var param = new Dictionary<string, object>
            {
                { "MasterApplicationLicenseId", request.MasterApplicationLicenseId ?? (object)DBNull.Value },
                { "ActivityCategoryId", request.ActivityCategoryId },
                { "TotalLicenses", request.TotalLicenses },
                { "CostPerLicense", request.CostPerLicense },
                { "BillingCycle", request.BillingCycle ?? (object)DBNull.Value },
                { "CreatedBy", request.CreatedBy }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_UpsertMasterApplicationLicense,
                param,
                out errorMessage);
            ...
            if (!string.IsNullOrWhiteSpace(errorMessage))
            {
                var errorAuditRequest = new MasterAuditLogRequestModel
                {
                    ...
                    EntityId = request.MasterApplicationLicenseId?.ToString(),
        ...
        public int Delete(DeleteRequest deleteRequest, out string errorMessage)
        {
            ...
            var param = new Dictionary<string, object>
            {
                { "MasterApplicationLicenseId", deleteRequest.Id },
                { "DeletedReason", deleteRequest.Reason },
                { "DeletedBy", deleteRequest.DeletedBy}
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_DeleteMasterApplicationLicense,
                param,
                out errorMessage);
