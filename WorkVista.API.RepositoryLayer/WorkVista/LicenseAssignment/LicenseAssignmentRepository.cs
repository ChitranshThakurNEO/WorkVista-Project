...
namespace WorkVista.API.RepositoryLayer.WorkVista.LicenseAssignment
{
    public class LicenseAssignmentRepository : ILicenseAssignmentRepository
    {
        ...
        private const string EntityName = AuditConstants.Entities.LicenseAssignment;

        public LicenseAssignmentRepository(IOptionsMonitor<AppSettingsModel> options, IMasterAuditLogRepository masterAuditLogRepository)
        {
        ...
        public List<LicenseAssignmentResponseModel> GetAll(out string errorMessage)
        {
            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetAllLicenseAssignments,
                new Dictionary<string, object>(),
                out errorMessage);

            return dt.ToList<LicenseAssignmentResponseModel>();
        }
        ...
        public LicenseAssignmentResponseModel GetById(int id, out string errorMessage)
        {
            ...
            {
                { "LicenseAssignmentId", id }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_GetLicenseAssignmentById,
                param,
                out errorMessage);

            if (dt.Rows.Count > 0)
                return dt.ToList<LicenseAssignmentResponseModel>().FirstOrDefault();
        ...
        public int AddOrUpdate(LicenseAssignmentRequestModel request, out string errorMessage)
        {
            ...
            var action = request.LicenseAssignmentId.HasValue && request.LicenseAssignmentId > 0
                ? AuditConstants.Actions.Update
                : AuditConstants.Actions.Insert;

            var param = new Dictionary<string, object>
            {
                { "LicenseAssignmentId", request.LicenseAssignmentId ?? (object)DBNull.Value },
            ...
            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_UpsertLicenseAssignment,
                param,
                out errorMessage);
            ...
            if (!string.IsNullOrWhiteSpace(errorMessage))
            {
                var errorAuditRequest = new MasterAuditLogRequestModel
                {
                    ...
                    EntityId = request.LicenseAssignmentId?.ToString(),
        ...
        public bool Release(int id, DateTime releasedDate, int createdBy, out string errorMessage)
        {
            ...
            _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_ReleaseLicenseAssignment,
                param,
                out errorMessage);
        ...
        public int Delete(DeleteRequest deleteRequest, out string errorMessage)
        {
            ...
            var param = new Dictionary<string, object>
            {
                { "LicenseAssignmentId", deleteRequest.Id },
                { "DeletedReason", deleteRequest.Reason },
                { "DeletedBy", deleteRequest.DeletedBy }
            };

            var dt = _sqlDbUtilities.ExectuteStoredProcedure(
                WorkVistaStoreProcedures.WorkVista_DeleteLicenseAssignment,
                param,
                out errorMessage);
